# AI Optimizer 配置模版 —— Benchmark 套件 (benchmark_runner 专用)
#
# 与 aiopt_conf.toml.tpl 的差异仅:
#   - optimizer_type = "llm"                (Benchmark runner 走 LLMOptimizer/MCTS)
#   - default_plan_timeout_seconds = 300.0  (Core-Set 多大查询, 单条超时 300s)
#   - remote_cache_enabled = true / cap_cache_timeout_by_baseline = true (与 .tpl 默认一致)
#   其余全部与 aiopt_conf.toml.tpl 默认相同。
#
# 用法:
#   1) 复制为正式配置:   cp mcts_scripts/benchmark/aiopt_conf.benchmark.toml.tpl etc/aiopt_conf.toml
#   2) 唯一必改项: 填好 [mcts].llm_api_url_key (其余开箱即用)
#   3) 直接跑:           python mcts_scripts/benchmark/benchmark_runner.py \
#                            --bench-json mcts_scripts/benchmark/core_set_1000_cdb.json
#   (runner 默认每实例 4 线程并行 + 优化后单线程验证; 默认配置路径仍是 etc/aiopt_conf.toml)
...
[optimizer]
optimizer_type = "llm"                       # Benchmark 套件走 LLMOptimizer/MCTS
...
default_plan_timeout_seconds = 300.0         # Benchmark 套件: baseline 探测/计划评估超时 300s
